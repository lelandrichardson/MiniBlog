...
namespace MiniBlog.Includes.Data
{
    ...
    public static class SqlMapperExtensions
    {
        ...
        /// <summary>
        /// inserts data into the specified table with an object mapped with proper field names and values
        /// author: lmr 6/5/12
        /// </summary>
        /// <param name="tableName">Schema Qualified name of Table</param>
        /// <param name="entityToInsert">anonymous/dynamic object with data to insert</param>
        /// <returns>identity of the inserted data</returns>
        public static long Insert(this IDbConnection connection, string tableName, dynamic entityToInsert, IDbTransaction transaction = null, int? commandTimeout = null)
        {
            var sbColumnList = new StringBuilder(null);
            var sbParameterList = new StringBuilder(null);

            int i = 0;
            foreach (System.ComponentModel.PropertyDescriptor property in System.ComponentModel.TypeDescriptor.GetProperties(entityToInsert as object))
            {
                if (i != 0)
                {
                    sbColumnList.Append(", ");
            ...
            int id = adapter.Insert(connection, transaction, commandTimeout, tableName, sbColumnList.ToString(), sbParameterList.ToString(), new List<PropertyInfo>(), entityToInsert);
            return id;
        }
...
public class SqlServerAdapter : ISqlAdapter
{
	public int Insert(IDbConnection connection, IDbTransaction transaction, int? commandTimeout, String tableName, string columnList, string parameterList, IEnumerable<PropertyInfo> keyProperties, object entityToInsert)
	{
		...
		connection.Execute(cmd, entityToInsert, transaction: transaction, commandTimeout: commandTimeout,commandType:CommandType.Text); 

		//NOTE: would prefer to use IDENT_CURRENT('tablename') or IDENT_SCOPE but these are not available on SQLCE
		var r = connection.Query("select @@IDENTITY id", transaction: transaction, commandTimeout: commandTimeout, commandType:CommandType.Text);
